...
def _build_rst_file(package_name: str = "bi_toolbox") -> None:
...
def _package_namespace(package_name: str) -> str:
    ...
    Returns:
        modified package_name: The modified package name.
    """
    return package_name.replace("-", "_")


def _package_dir(package_name: str = "bi_toolbox") -> Path:
...
def _get_package_version(package_dir: Path) -> str:
    ...
    try:
        with open(package_dir.parents[1] / "pyproject.toml", "r") as f:
            pyproject = toml.load(f)
    except FileNotFoundError as e:
        print(
            f"pyproject.toml not found in {package_dir.parents[1]}.\n"
...
def _build_index(dirs: List[str]) -> None:
    custom_names = {
        "bi_toolbox": "BI Toolbox",
    }
    ordered = ["bi_toolbox"]
    main_ = [dir_ for dir_ in ordered if dir_ in dirs]
    integrations = sorted(dir_ for dir_ in dirs if dir_ not in main_)
    doc = """# BI Toolbox Reference

Welcome to the BI Toolbox reference.

For user guides see [https://bi-toolbox.readthedocs.io](https://bi-toolbox.readthedocs.io).
...
def main(dirs: Optional[list] = None) -> None:
    ...
    print("Starting to build reference files.")
    if not dirs:
        dirs = [
            dir_
            for dir_ in os.listdir(ROOT_DIR / "src")
            if dir_ not in ("tests", "scripts", "placeholder")
        ]
        dirs += [
            dir_
            for dir_ in os.listdir(ROOT_DIR / "src")
            if os.path.isdir(ROOT_DIR / "src" / "placeholder" / dir_)
            and "pyproject.toml" in os.listdir(ROOT_DIR / "src" / "placeholder" / dir_)
        ]
    for dir_ in dirs:
